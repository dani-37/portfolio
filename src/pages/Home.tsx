...
export default function Home() {
  ...
  function scrollToSection(sectionIndex: number) {
    ...
    // Just past the wipe completion so section is fully visible
    const target = (sectionIndex + 1) / numClips;
    window.scrollTo({ top: target * scrollable, behavior: "instant" });
  }
  ...
  function handleBack() {
    ...
    if (detail?.type === "experience") {
      scrollToSection(0); // Experience is clip index 0
    } else if (detail?.type === "project") {
      scrollToSection(1); // Projects is clip index 1
    } else {
      ...
      overlay = (
        <div className="absolute inset-0 overflow-y-auto">
          <article className="max-w-2xl mx-auto px-6 pt-8 pb-16 font-grotesk font-light text-ink">
            <p className="font-mono text-[8px] tracking-[3px] uppercase text-[#aaa] mb-2">
              {role.years}
            </p>
            <h1 className="font-syne font-bold text-[40px] tracking-[-1.5px] leading-none mb-4">
              {role.name}
            </h1>
            <p className="font-grotesk font-light text-[12px] text-[#999] mb-8">
              {role.role}
            </p>
    ...
    if (project) {
      overlay = (
        <div className="absolute inset-0 overflow-y-auto">
          <Suspense
            fallback={
              <p className="p-10 font-mono text-[9px] text-[#aaa] tracking-[2px]">
                loading...
              </p>
            }>
    ...
    <>
      ...
      <div className="hidden md:block">
        <Card cardRef={cardRef} clipRefs={clipRefs} overlay={overlay}>
          <Hero />
          <Experience
            onSelect={(id) => openDetail({ type: "experience", id })}
          />
          <ProjectsList
            onSelect={(slug) => openDetail({ type: "project", slug })}
          />
          <Contact />
        </Card>
        ...
        {!detail && (
          <>
            ...
            <div
              ref={navRef}
              className="fixed z-20 flex flex-col items-end gap-[2px]"
              style={{
                top: "50%",
                right: "20px",
            ...
            >
              {["Hero", "Experience", "Projects", "Contact"].map((label, i) => (
                <div
                  key={i}
                ...
                >
                  <span
                    className="font-mono text-[8px] tracking-[2px] uppercase text-[#888] opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none select-none"
                  >
                    {label}
                  </span>
                  <div
                    style={{
                      width: i === 0 ? "32px" : "20px",
                      height: "1.5px",
                      background: i === 0 ? "#1a6b5a" : "#9aaba3",
                      transition: "width 0.5s cubic-bezier(0.4, 0, 0.2, 1), background-color 0.5s cubic-bezier(0.4, 0, 0.2, 1)",
                    }}
                  />
                </div>
              ))}
        ...
        <div
          ...
          {detail ? (
            <div
              ...
              style={{ background: "rgb(253, 252, 249)" }}>
                ...
                (() => {
                  ...
                  return (
                    <div className="p-8 pb-10">
                      ...
                      <h2 className="font-display font-bold text-[28px] tracking-[-1px] text-ink leading-none mb-2">
                        {role.name}
                      </h2>
                      <p className="font-grotesk font-light text-[12px] text-[#999] mb-6">
                        {role.role}
                      </p>
                      <div className="space-y-4 font-grotesk font-light text-[14px] text-ink leading-relaxed">
                        {role.detail}
                      </div>
                    </div>
                  );
              ...
              {detail.type === "project" &&
                (() => {
                  ...
                  return (
                    <Suspense
                      fallback={
                        <p className="p-8 font-mono text-[9px] text-[#aaa] tracking-[2px]">
                          loading...
                        </p>
                      }>
                      <project.Component stack={project.stack} />
            ...
            <div className="h-full overflow-y-auto snap-y snap-mandatory">
              {[
                ...
                <div
                  key={i}
                  className="snap-start relative shrink-0"
                  style={{ height: "min(80vh, 460px)" }}>
